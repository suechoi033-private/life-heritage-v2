...
**seed-16 웰다잉 가이드 콘텐츠 초안 작성** (카피라이터 세션, 사장님 요청)
- `docs/content/seed-16-well-dying-guide.md` 신규 — "웰다잉(Well-Dying) — 좋은 죽음을 미리 준비한다는 것"
- lifecycle 2단계(사별 경험 후 자기 준비 시작) + 3단계(자기 준비) 교차 진입, 40대(부모 준비 지원) 보조.
- 구성: 공감 도입(한국의 죽음 금기·준비 없이 맞는 현실) → 웰다잉의 본질(한보사연 2018–2019 연구 인용, 죽음 준비가 아닌 삶의 재확인) → 시작 시점 근거(50대 권고, 아직 건강할 때 = 가족 배려) → 실천 5가지(가족 대화·사전연명의향서·가치 성찰·유언·디지털 정리·실질 준비) → 웰다잉의 심리적 의미(가벼워짐·지금 집중) → 참고 자료 섹션(서울대 웰다잉프로젝트·한보사연) → 잇다 CTA 4종 → 체크리스트 7항목.
- seed-02(사전연명의료의향서) 내부 링크 연결. 서희정 교수는 미확인 인용 금지 원칙에 따라 "참고 자료" 섹션으로 분리.
- 약 2,700자. YMYL 안내문 삽입, 감수자 미배정.
...
- 마무리 행동 제안: "이사 가능 시점 + 월 비용"을 종이 한 장에 적는 것 — 결정 마비 없이 시작할 수 있는 최소 행동으로 압축. 약 2,980자.
