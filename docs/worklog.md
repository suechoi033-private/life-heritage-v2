...
**진단: "살아 있을 확률"·자기성찰 콘텐츠 노출 안 됨 — 지면 변경 영향** (마케팅 세션, 사장님 제보)
- 사장님 제보: 잇다 콘텐츠가 노출 안 됨, 지면 바뀐 탓인지 / 자기성찰·"살아있을 확률" 노출 자리 어디였나.
- 원격 DB 직접 확인. 두 갈래로 갈림:
  - **자기성찰 시리즈("내일 다시 못 깨어난다면")** = 홈 최상단 카드 + `reflection.html`, **둘 다 회원 전용**(`index.html:490` `if(user)`). 마이그레이션 `20260614_reflection_series.sql`은 적용됨(DB에 8문항 확인) → 안 보이면 **로그아웃 상태로 보거나, 이미 답해 잔잔한 "다시 보기" 상태**.
  - **"살아 있을 확률" 외 6편(오늘잇고 큐레이션, `category='reflection'`, hero_day 1~7, is_published=true)** = 발행돼 있으나 **홈 피드가 reflection 카테고리를 의도적 제외**(`js/content.js:103` `.neq('category','reflection')`, 2026-06-07 bbd7585 도입) + 히어로는 `format='story'`만 받는데 이들은 `article` → **라이브 어디에도 안 뜸**. 사장님 직관("지면 바뀜") 정확. `prototype-forest.html`엔 큐레이션 히어로로 살아있으나 프로토타입(네비 미연결).
  - **결정 필요(사장님)**: 이 7편 큐레이션 에세이를 라이브에 되살릴지 / 되살린다면 어디(전용 "오늘 잇고" 큐레이션 지면 신설 vs 홈 피드 reflection 제외 해제). 지면/PE 결정.
  - 사장님 선택: **전용 "오늘 잇고" 지면 신설 방향 — 디자인 목업 요청.** → 실제 디자인 토큰으로 화면 목업 제작(Playwright 렌더 PNG): 마니페스토 + 오늘의 큐레이션 히어로("살아 있을 확률", 출처 표기) + 나에게 닿는 질문(읽기→기록 흐름) + 지난 잇고 리스트(고아 6편) + 하단 네비(콘텐츠 탭 활성). 소스: `docs/product/onneul-itgo-surface-mockup.html`. 구현 여부는 사장님 컨펌 후 PE.
  - **지면 할당 흐름도 제작**(사장님 추가 요청): ① 홈 진입(기존 🕯오늘잇고 카드 링크만 변경, 추가 0) → ② 전용 지면(신설 1개, 고아 7편의 집) → ③ 에세이 상세(content-detail 재사용). 홈 피드 reflection 제외 필터는 유지(충돌 0). 소스·PNG: `docs/product/onneul-itgo-allocation-flow.html`.

**인스타 카드뉴스 "살아 있을 확률" 8장 제작** (마케팅 세션, 사장님 요청)
- 노출 자리 잃은 큐레이션 에세이를 인스타 organic 채널로 재활용하는 첫 편. seed-12 포맷·톤 계승(흰 배경·에버그린·나눔명조·무약속 CTA·자극 0).
- hook(통계) → 숫자 → 비유 → 우리가 놓치는 것 → 전환("위로 아니라 통계") → 나에게 닿는 질문 → 잇다 소개 → CTA. 출처 「편안함의 습격」 마이클 이스터 명시.
- 산출물: `docs/marketing/insta-alive-probability-cardnews-2026-06-14.md`. 실 이미지·게시는 사장님 승인 후. hero_day 2~7 6편 시리즈 확장 제안 포함.
