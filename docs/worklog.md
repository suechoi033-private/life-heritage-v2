...
**묻다 오늘의 질문 — "저장 버튼이 안 눌려요" 대응** (사장님 제보)
- 진단: API 로그상 페이지 로드 요청 후 저장 시 나가야 할 insert 요청이 서버에 미도달 — 탭이 핸들러에 닿지 않거나 실패가 무표시.
- 수정(`mutda/question.html`): ①누르면 즉시 "저장 중…" + 버튼 잠금(눌림 가시화) ②fetch가 던지는 경우도 catch해 네트워크 안내 토스트 ③`question_save_click` 이벤트 추가 — 탭 도달 여부를 서버에서 추적 가능 ④button type="button" 명시.
- `mutda/sw.js → mutda-v27`. 재현 시 이벤트 로그로 (탭 미도달 vs 저장 실패) 이분 진단 가능해짐. → 사장님 재시도 후 정상 저장 확인(DB 검증).
- 오늘의 질문 하단 "다른 질문들" 목록 제거 (사장님 지시 — 질문은 매일 롤링으로만). sw → v28.

## 2026-07-29 백필

**묻다 오늘의 질문 저장 — 원인 확정·자동 재시도** (사장님 재제보 "저장 중…에서 멈춤")
- 원인 확정: 탭은 정상, 모바일에서 저장 요청이 수십 초 지연되는 케이스 (이전 "안 눌려"도 동일 원인 추정 — 당시엔 피드백이 없어 무반응으로 보임).
- 수정: 저장 로직 saveOnce로 분리 + 8초 타임아웃 → 자동 재시도 1회(15초) → 그래도 느리면 안내 토스트(입력 보존). 재시도 중복 insert는 유니크(23505)→update 폴백이 흡수. `question_save_failed` 이벤트로 실패 사유 추적. sw → v29.
- 오늘의 질문 25번 추가 (창업자 지정): "이 세상에서 가장 소중한 것은 무엇인가요?" — 라이브 선적용 + 멱등 마이그레이션 기록(`20260729_mutda_question_most_precious.sql`).
...
**묻다 QR 명함 제작** (창업자 요청)
- `docs/marketing/mutda-business-card.html` — 국내 표준 90×50mm + 2mm 도련(94×54mm) 인쇄용 2페이지.
  앞면 브랜드/연락처(브라우저에서 클릭 수정 가능), 뒷면 딥그린 배경 + QR.
- QR은 묻다 랜딩 `?from=card` 링크 — 명함 유입이 `landing_view`에 별도 집계됨. 렌더 후 디코드 검증 완료.
- 창업자 확정 반영: 최수련 · 010-4388-1708, 로고 마침표·직함 제거.
- 태그라인 확정(창업자 작): "마지막 순간에 “인생이 참 좋았다. 잘 살았다” 말하기 위해, 죽음을 묻고 생각합니다."
- 디지털 명함(모바일 한 장) 추가: 세이지 그린 A안 + 하이브리드 폰트(로고·태그라인 명조, 정보 IBM Plex Sans KR),
  QR 2개(묻다 둘러보기 `?from=card` · 저서 『타인의 미래』 알라딘). 렌더 디코드 검증.
- 인쇄용은 비용 고려 흰 바탕으로 전환(뒷면 딥그린 제거). 렌더링 검증 시 샌드박스 구글폰트 차단 발견 → 로컬 폰트로 검증 체계 수정.

## 2026-07-16

**묻다 — 어카운트인포 모바일 오류 대응** (사장님 제보 "은행보험증권 어카운트 인포 오류나네")
- 🐞 증상: `mutda/digital.html` 금융 섹션의 payinfo.or.kr 링크를 휴대폰에서 열면 "오류가 발생하였습니다" — 어카운트인포 웹사이트는 **PC 전용**, 모바일은 전용 앱만 지원(웹 검증 완료).
- 수정: 금융 섹션 안내를 기기별로 분리 — 모바일은 '어카운트인포' 앱 설치(Google Play `com.kftc.payinfo.android` 링크, 아이폰은 App Store 검색 안내), PC는 www.payinfo.or.kr. 구독 섹션 인라인 링크도 동일 취지로 수정.
- `mutda/sw.js → mutda-v23-2026-07-16-payinfo-mobile-guide`.

## 2026-07-10

**묻다 — 디지털·금융 정리 위저드 + 콘텐츠 소속 재편** (사장님 결정 3건 실행)
- 사장님 결정: ①디지털 계정 정리 위저드 구현 ②어카운트인포 기록 가이드 구현 ③**이원화 금지 — 죽음준비 콘텐츠는 묻다 소속, 잇다는 돌봄케어링**(전략팀 "형식 변환" 안 기각).
- **신규 `mutda/digital.html`** (여정 매듭 '디지털·금융 정리' 🔑): ①금융 — 어카운트인포(payinfo.or.kr)·내보험다보여 딥링크 + 계좌·보험 위치·힌트 기록 ②디지털 계정 — 플랫폼 6종 프리셋: 구글(휴면 계정 관리자)·애플(디지털 유산 연락처)·페북(기념계정 관리자)은 **딥링크+설정했어요 체크**, 인스타·네이버·카카오는 "사전 설정 없음→유족이 할 일" 배지 + 폐쇄/추모/이관/유지 의사 기록 ③구독·자동이체 ④**"가족에게 남겨질 안내" 자동 요약**(안심상속 원스톱 안내 포함). 비밀번호 저장 금지 고지.
- DB `20260710_mutda_digital_items.sql`: kind(finance/account/subscription)·platform_key(프리셋 유니크)·decision·note·preset_done, owner RLS. journey.js 매듭+완료판정 추가.
- **P0 — 묻다 아티클 비회원 공개**: `initPublicPage()` 신설(로그인 비강제, 로그인 시 스트릭·하트비트만) — library + article 2편 적용. 브런치 소프트 랜딩 가능해짐.
- **콘텐츠 소속 이동(1차, 노출 기준)**: 사전연명의료의향서·웰다잉·상조 가입 → 묻다 길잡이 카드로(파일은 리스킨 전까지 기존 경로 링크), 잇다 info 허브(PILLARS·시점별·ENABLED_KEYS)에서 제거. 자택임종·납골당·실버타운·요양·호스피스는 잇다 유지(돌봄·유족 절차). '미리' 탭의 '디지털 정리' 항목도 묻다로.
- **잇다 디지털 자산 노트 이관**: `note/digital.html` → 묻다 digital.html 리다이렉트 스텁, self.html 카드 목적지 변경(localStorage 기록은 유령 서버저장이었음 — 서버 저장으로 승격).
- sw 양쪽 범프(itda deathprep-to-mutda / mutda digital-wizard).

## 2026-07-09

**묻다 콘텐츠 user journey 설계 + 사후 계정·상속 준비 사실검증** (사장님 의제, 전략 세션 — 프로덕트 미반영)
- 산출물: `docs/strategy/mutda-content-journey-2026-07.md` (297줄). 핵심: 묻다 콘텐츠 = "읽을거리"가 아니라 **매듭에 붙는 실천 가이드**(7~9편 캡). 배치는 여정 매듭 인라인(시작 전 3분 읽기 + 완료 순간 다음 매듭 카드) 주축 + 길잡이 서가 보조, 홈 피드는 기각(조용함 원칙). 분류 판정 3문(주어가 나/부모·미리/이후·CTA가 묻다/잇다 도구). 정책 = 이관·중복 아니라 **정본 1곳 + 형식 변환**(브런치=왜, 묻다=어떻게). 브런치 CTA `?ref=brunch_*` 태깅(referral.js 기구현).
- 조사 중 결함 2건 발견: ①**묻다 아티클이 로그인 필수**(initAppPage) — 브런치 독자 소프트 랜딩 불가, 비회원 공개 전환 P0 제안 ②기게시 브런치 원고 9편의 CTA가 전부 잇다행(투브랜드 이전 작성) — 09번 목적지 will-start는 이미 묻다 리다이렉트 스텁이라 이중 점프.
- 사실검증(YMYL, 웹 검증 완료): 사후 계정 사전설정 = 구글(휴면 계정 관리자)·페북(기념계정 관리자)·애플(디지털 유산 연락처) ✅ / 인스타·네이버·카카오 ❌(사후 유족 요청만, 네이버는 비공개정보 미제공·카카오는 경제적 가치 자산만 상속). 마이데이터 연동 = 본인신용정보관리업 허가 필요 → 불가, 대안 = **어카운트인포·내보험다보여 생전 조회 → 묻다 기록** 가이드. **안심상속 원스톱 = 사후 상속인 신청 제도(사망신고 시~상속개시월 말일부터 1년), 생전 가입 개념 없음** — 대비책은 생전 기록 + "유족에게 남기는 안내" 카드.
- 창업자 결정 대기(문서 §6): 브런치 기게시 CTA 수정 / 사전연명의향서 묻다 이관 / well-dying 브리지 / 상속 가이드는 변호사 감수 전 발행 금지.
...
- 구현(즉시 가능, 활용신청 불필요): 각 카드에 **"🔎 홈페이지·지도 찾기"**(기관명+지역 네이버 검색, 홈페이지·전화·지도·후기 일괄) + 전화 데이터가 들어오면 **"📞 전화"** 버튼(`tel:`). 기존 `.facility-actions`·`.btn-compare` CSS 재활용(비교담기 JS 제거 후 잔존 스타일).
- `sw.js → itda-v4-2026-07-09-nursing-home-naver-tel`. 프런트만 변경(Pages 자동배포).
- 사장님 액션(선택): 상세조회 활용신청 시 카드에 전화·주소·정원 직접 표시 자동 활성.
...
- 신규 사업 결정 기록(잇다 외): 독서큐레이션 서비스명 **일독**("일하다 막히면, 일독을 권합니다") —
  미배포 앱 대시보드는 런칭일에 스니펫+SITES 1줄로 합류하기로(관제탑 단일 유지).

**묻다 편지 전하기에 초대 링크** (창업자 제안)
- 📨 전하기로 나가는 카톡/문자 끝에 묻다 랜딩 링크(`?from=letter`)와 답례 제안 문구 추가 —
  감사는 "당신도 고마운 사람에게, 늦기 전에", 작별은 "남기고 싶은 말을 미리". 받은 사람이 그대로 들어와 자기 마음을 전하는 바이럴 루프.
- 랜딩 `landing_view` 이벤트에 `from` 유입 경로 기록 — 편지발 유입 측정 가능.

**묻다 디자인 스케일 전면 재조정 — '실버앱' 느낌 제거** (사장님 피드백 + 자체 재설계)
- 원인 진단: 잇다에서 물려받은 전역 110% 확대(zoom 1.1)가 주범 → 묻다는 표준 스케일로 분리 (타겟 30~60대, CSS에 결정 주석). 글로벌 death-tech(Lantern·Cake·Empathy) 문법 벤치마킹: 세리프 헤드라인 + 작은 컨트롤.
- 버튼 12px/15px·여정 매듭 40→28px·칩/타이포/입력/내비 전반 축소 (입력 16px은 iOS 자동확대 방지 유지). 로고 '묻다.'→'묻다' 전 페이지. 오늘의 한 걸음 CTA 내용폭. 탭 '서재'→'길잡이'(🧭). 자매서비스 카드 한 줄 컴팩트(길잡이·나).
- 편지 흐름 재설계: 작성창 기본 접힘 → 오늘의 질문/새로 쓰기/수정에서 열림, 질문이 작성창에 인용 표시, '첫 문장 받기'는 막힐 때 보조로. '열기'→'✏️ 수정' + 수정 저장/닫기 — 수정 기능 명확화.
- 전날: 모바일 320px에서 목록이 버튼에 짓눌리는 문제 수정(.item-row wrap + 본문 최소폭, 회귀 스크립트 추가), 계정 합치기(공산→단청) 완료, Q3 목표 문서·Flutter 전환 계획(주간 자동 점검 트리거)·묻다 기획서 v2·잇다 케어 개편 1단계·묻다↔잇다 브리지.
