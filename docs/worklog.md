...
**추모 피규어(사진→3D) 국내 제작 파트너 소싱 자료 신설 — `docs/product/memorial-figurine-sourcing.md`** (사장님 세션: kaiven.shop 보고 "잇다에서 팔 방법")
- kaiven.shop/memofigurines.com 류는 중국 공장(Sailner 풀컬러) 제품을 파는 **드롭쉬핑**임을 조사로 확인 — 기술·공장은 누구나 접근 가능, 진입장벽 낮음.
- 사장님 선택: **국내 업체 화이트라벨 제휴부터**(품질·CS·배송 통제 우선, 잇다 톤 적합). 중국 드롭십(B)·직접 제작(C)은 참고로만.
- 산출물: 먼저 연락할 업체 숏리스트(이오이스·케이스타피규어·몬스터3D·마리엘피규어), **제휴/견적 문의 메일 복붙 초안**, 샘플 평가 체크리스트, 다음 단계.
- 톤 경고 명시: kaiven식 할인·긴급·경쟁 퍼널은 헌장 위반 → "곁에 두는 작은 형태"로. 추모는 현 보류·숨김 상태, 사별 맥락 품질 미달 시 정서적 타격 큼 → 품질 통제가 성패.
- docs만 변경(앱/화면 무관, CACHE_VERSION 불필요). 후속: 업체 회신·샘플 비교 → 파트너 확정 → 잇다 상품 페이지+사진 업로드 플로우 구현.
