...
**글로벌 경쟁 지형 전수 조사 신설 — `docs/benchmark/competitive-landscape.md`** (전략 세션, 사장님 의제)
- 한국·미국·일본·유럽·이스라엘 시장을 A~K 11개 기능 축으로 전수 매핑(유언/상속·사전연명·디지털vault·가족케어협업·복약·요양매칭·장례상조·그리프커뮤니티·종활종합·회고록·한국통합).
- 새로 부각된 직접 경쟁자 Top5: **Empathy(US/IL, $90M+)·케어닥·고이·鎌倉新書·Cake.** Empathy를 잇다와 철학이 가장 닮은 reference로 식별(단 after-loss 한 시점만).
- White space 글로벌 데이터로 재확인·확장(strategy 9절): #1 lifecycle 통합 #2 형제 협업 케어저널 #3 사별 전→후 다리 — 3개가 ★★★(글로벌 전무). 단 "아무도 안 함 ≠ 수요 검증됨" 경고 병기.
- 한국 시장 결론: 단계별 강자(케어링 750억·케어닥 315억·고이)는 있으나 **lifecycle 통합자는 공백**. 다만 한국은 종활 카테고리가 일본만큼 성숙하지 않아 통합 가치 자체가 미검증 가정.
- 링크는 검증된 도메인만 기재, 미검증은 "미확인"/추정은 "[추정]" 표기.
