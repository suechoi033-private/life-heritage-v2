...
**설계기록(설계 디자인 문서) 도입 검토 → 미래 트리거만 남김** (사장님 요청)
- 사장님 질문: 오늘의집 Remicon 류 Google-style 설계문서를 잇다 코딩 전·중·후에 상시 작성하면 도움이 될까. **결론: 지금은 도입하지 않는다.** 잇다는 정적 HTML+바닐라 JS+Supabase·빌드 없음·실질 단일 결정자 → 그 무게의 문서 프로세스는 헌장 "안 아픈 곳에 시스템 짓지 않기/검증 우선"에 어긋남. 그 포맷의 핵심(왜·결정·door유형·재검토 트리거·날짜 로그)은 이미 `docs/decisions/` ADR 패턴 + `docs/worklog.md`가 분담 중.
- 사장님 지시: "지금은 하지 말고, 잇다가 커지고 복잡해지면 적절한 시점에 제안." 세션은 컨테이너가 매번 새로 떠 대화를 기억 못 하므로, 약속을 보존하려면 레포에 표식이 필요 → 트리거만 기록.
- `docs/decisions/0001-content-management.md` 맨 아래 **📌 미래 트리거 — 설계기록(ADR 상시화) 도입 재검토** 블록 추가. 트리거: ①코드 기여자 창업자 외 2명↑ ②동시 진행 기능 3개↑ 또는 세션 간 충돌·되돌리기 사고 발생 ③Supabase 스키마/RPC 되돌리느라 시간 날린 사건 발생 ④외부 API/연동 여러 팀 공유 시작. 충족 전엔 손대지 않음.
