...
- 한꺼번에가 아니라 한 명씩 알음알음 초대하는 방식에 맞춰, 고정 `test_start` 코호트를 폐기.
- `/retention-report` 명령을 "가입한 실사용자 전원(=초대한 사람) 누적, 사람마다 가입 후 N일째,
  재방문(D1+)·2회차·초대" 틀로 재작성. 재방문 판정을 "가입일보다 뒤의 날 활동 존재"로 정밀화.
- 운영자/Admin/중복 계정은 리포트에서 분리 표시하도록 절차에 명시.
