// 묻다 — 유입 경로 트래킹. ?ref= 파라미터를 localStorage에 캡처 → 가입 완료 시 profiles로 이관.
// 잇다와 같은 오리진(github.io/life-heritage-v2)·같은 Supabase profiles 테이블을 쓰므로
// localStorage 키(itda:referral_source)를 공용으로 둔다 — 어느 서비스로 들어오든 first-touch 보존.
// 사용:
//   진입 페이지(index/signup/login.html): import { captureReferral } from './js/referral.js'; captureReferral();
//   가입 완료 훅(welcome.html):            import { hoistReferral } from './js/referral.js'; await hoistReferral(supabase, user.id);
